...
const trustBadges = [
  { icon: "/beneficio-familiar-assets/tax-free.svg", text: "Retiro libre de impuestos" },
  { icon: "/beneficio-familiar-assets/family-protection.svg", text: "Protege a tu familia" },
  { icon: "/beneficio-familiar-assets/minutes.svg", text: "Toma menos de 2 minutos" },
];

const questionnaireSecuritySeals = [
  {
    src: "/beneficio-familiar-assets/busines-acredited-bbb.avif",
    alt: "BBB Accredited Business",
    width: 112,
    height: 38,
  },
  {
    src: "/beneficio-familiar-assets/secure-form-beneficio-familiar.png",
    alt: "Secure Form",
    width: 136,
    height: 32,
  },
  {
    src: "/beneficio-familiar-assets/ssl-encription.avif",
    alt: "SSL Encryption",
    width: 112,
    height: 38,
  },
...
const footerLinks = [
  { label: "About Us", href: "/v4-rafa" },
  { label: "Cookie Policy", href: "/v4-rafa" },
  { label: "Terms Of Use", href: "/v4-rafa" },
  { label: "Partner With Us", href: "/v4-rafa" },
  { label: "Privacy Policy", href: "/v4-rafa" },
  { label: "Contact", href: "/v4-rafa" },
  { label: "Sitemap", href: "/v4-rafa" },
];

const socialLinks = [
  { label: "Facebook", href: "https://www.facebook.com/BeneficioFamiliar", icon: "/beneficio-familiar-assets/facebook.png" },
  { label: "Instagram", href: "https://www.instagram.com/", icon: "/beneficio-familiar-assets/instagram.png" },
  { label: "LinkedIn", href: "/v4-rafa", icon: "/beneficio-familiar-assets/linkedin.png" },
];
...
const ageRejectedCookieMaxAge = 90 * 24 * 60 * 60;
const rejectedAgeOption = "65+";
...
function hasAgeRejectedCookie() {
  ...
  return document.cookie
    .split("; ")
    .some((cookie) => cookie === `${ageRejectedCookieName}=true`);
}

function setAgeRejectedCookie() {
  if (typeof document === "undefined") return;
  document.cookie = [
    `${ageRejectedCookieName}=true`,
    `Max-Age=${ageRejectedCookieMaxAge}`,
    "Path=/",
    "SameSite=Lax",
  ].join("; ");
}
...
type MetaTrackingWindow = Window &
  typeof globalThis & {
    ...
    _fbq?: MetaTrackingWindow["fbq"];
    __v4RafaMetaPixelId?: string;
    __v4RafaPixelBootstrapped?: boolean;
    __v4RafaPageViewTracked?: boolean;
...
function ensureMetaPixel(trackPageView = false) {
  ...
  if (typeof trackingWindow.fbq !== "function") {
    const fbq = function (...args: unknown[]) {
      ...
      fbq.queue?.push(args);
    } as NonNullable<MetaTrackingWindow["fbq"]>;
  ...
  if (!trackingWindow.__v4RafaPixelBootstrapped) {
    trackingWindow.__v4RafaPixelBootstrapped = true;
    trackingWindow.__v4RafaMetaPixelId = metaPixelId;
...
export default function V4RafaClientPage({
  ...
  initialAgeRejected?: boolean;
}) {
  const [currentStep, setCurrentStep] = useState<FunnelStep>("age");
  const [isAgeRejected, setIsAgeRejected] = useState(
    () => initialAgeRejected || hasAgeRejectedCookie(),
  );
  ...
  useEffect(() => {
    if (isAgeRejected) return;
    trackMetaPageView();
    const delayedPageViewId = window.setTimeout(trackMetaPageView, 900);
    return () => window.clearTimeout(delayedPageViewId);
  }, [isAgeRejected]);
  ...
  }, [answers.zipCode, defaultLocationText, hasLoadedGeo, isAgeRejected]);
  ...
  }, [currentStep, isAgeRejected, successHash]);

  useEffect(() => {
    if (isAgeRejected) return;
    if (currentStep !== "state" || shouldAskZipCode) return;
    transitionTo("name", "forward");
  }, [currentStep, isAgeRejected, shouldAskZipCode]);

  useEffect(() => {
    if (isAgeRejected) return;
    if (currentQuestionIndex < 0) return;
    trackMetaViewContentStep(
      currentStep,
      currentQuestionIndex,
      visibleQuestionSteps.length,
    );
  }, [currentQuestionIndex, currentStep, isAgeRejected, visibleQuestionSteps.length]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("popstate", keepRejectedView);
  }, [isAgeRejected]);
  ...
  function rejectForAge() {
    setAgeRejectedCookie();
    setAnswers((prev) => ({ ...prev, ageGroup: rejectedAgeOption }));
    setCurrentStep("age");
    setPanelKey((prev) => prev + 1);
    setIsTransitioningOut(false);
    setSubmitError("");
    setIsAgeRejected(true);
  }
  ...
  function handleDirectChoice<K extends keyof FunnelAnswers>(
    ...
    if (field === "ageGroup" && value === rejectedAgeOption) {
      rejectForAge();
      return;
    }
      ...
      <div className="mx-auto w-full max-w-[490px] overflow-hidden bg-white">
        ...
        <section className="border-t-[3px] border-[#ffbe2e] bg-[#1e2639] px-5 py-8 text-center text-white md:px-6">
          ...
          <p className="mx-auto mt-6 max-w-[380px] text-[14px] leading-[1.6] text-white/55">
            Beneficio Familiar es una plataforma independiente de información sobre
  ...
  function renderAgeRejectedPage() {
    return (
      <section className="mx-auto flex min-h-[calc(100vh-60px)] w-full max-w-[760px] items-center justify-center px-4 py-10 text-center">
        <div className="w-full max-w-[520px] rounded-[18px] border border-[#e6edf6] bg-white px-6 py-9 shadow-[0_14px_34px_rgba(16,24,32,0.08)] md:px-10 md:py-11">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#eef5ff] text-[var(--brand)]">
            <FilledCheckIcon className="h-7 w-7" />
          </div>
          <p className="mt-5 text-[14px] font-extrabold uppercase tracking-[0.04em] text-[var(--brand)]">
            Gracias por tu interés
          </p>
          <h1 className="mt-3 text-[30px] leading-[1.12] font-bold tracking-[-0.04em] text-[#101820] md:text-[42px]">
            Por ahora no calificas para este beneficio
          </h1>
          <p className="mx-auto mt-4 max-w-[430px] text-[16px] leading-[1.55] text-[#5d6782] md:text-[18px]">
            Este programa está disponible solo para ciertos grupos de edad. Agradecemos tu tiempo y esperamos poder ayudarte con otras opciones más adelante.
          </p>
        </div>
      </section>
    );
            ...
            <div className={`mt-8 grid w-full max-w-[460px] gap-4 md:mt-10 ${animationClass}`}>
              {goalOptions.map((option) => (
                ...
                >
                  ...
                  ) : option === "Planificación de retiro" ? (
                    <span className="inline-flex items-center justify-center gap-2">
                      <RetirementPlanIcon className="h-[22px] w-[22px] text-[#5d6674]" />
                      <span>{option}</span>
                    </span>
    ...
    <main className="min-h-screen bg-[var(--page-bg)] text-[var(--ink)]">
      ...
      <header className="border-b border-black/6 bg-white/96 shadow-[0_6px_18px_rgba(18,31,53,0.08)] backdrop-blur-sm">
        <div className="mx-auto flex h-[60px] w-full max-w-[1200px] items-center justify-between px-4 md:relative md:justify-center">
          <Image
            src="/beneficio-familiar-assets/beneficio-familiar-logo.png"
            alt="Beneficio Familiar"
            width={180}
            height={48}
            priority
      ...
      <footer className="px-4 pb-5 pt-3 text-center text-[9px] leading-[1.45] text-[#b8bec8] md:text-[10px]">
        <p>© 2025 Beneficio Familiar. All Rights Reserved.</p>
        ...
        <p className="mx-auto mt-2 max-w-[920px]">
          Beneficio Familiar is an independent promotional and advertising service. This website and the services offered are not sponsored, affiliated with, endorsed, or administered by Facebook. The content on this site has not been reviewed, approved, or certified by Facebook or any of its related entities.
        </p>
      </footer>
    </main>
  );
